...
class CommandTransferMember : KingdomsParentCommand("transferMember", true) {
    ...
    override fun tabComplete(context: CommandTabContext): List<String> {
        val kingdomPlayer = context.kingdomPlayer ?: return emptyTab()
        val kingdom = kingdomPlayer.kingdom ?: return emptyTab()

        val players = kingdom.getMembers { player: KingdomPlayer ->
            if (player.rank.priority > kingdomPlayer.rank.priority) {
                return@getMembers player.player.name
            }
            ""
        }

        players.removeAll(setOf<Any?>(""))
